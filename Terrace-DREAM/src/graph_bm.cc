...
pair_uint* read_input(string filename, unsigned long int * edge_num, unsigned int *node_num) {
  ifstream f;
  string line;
  f.open(filename);
  if (!f.good()) {
  ...
  while (getline(f, line)) {
    ...
    edges.push_back(make_pair(src-1,target-1));
  }
  *node_num=num_nodes;
  *edge_num=edges.size();
  pair_uint *edges_array = (pair_uint *)malloc(edges.size() * sizeof(pair_uint));
  parallel_for (uint32_t i=0; i < edges.size(); i++) {
    edges_array[i] = {get<0>(edges[i]),get<1>(edges[i])};
  }
  return edges_array;
}
void run_algorithm(commandLine& P) {
  ...
  auto d_b = P.getOptionValue("-db", "none");
  int db=stoi(d_b);
  printf("db is %d\n",db);
  pair_uint* edges=read_input(filename,&num_edges, &num_nodes);
  printf("the number of nodes and edges are %d, %d\n",num_nodes,num_edges);
	...
	gettimeofday(&end, &tzp);
  print_time_elapsed("Inserting total edges: ", &start, &end);
  printf("-------------------------------\n");
  exit(-1);
  for(uint32_t i = 0; i < num_edges; i++) {
