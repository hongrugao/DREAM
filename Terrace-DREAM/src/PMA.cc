...
//TODO pass in subcounts and do redistibute_par when big
 /* void PMA::double_list(uint64_t task_id, std::vector<uint_t> &sub_counts, uint_t num_elements) {
  ...
  printf("here, double list sub_count is %d\n",sub_counts.size());
  ...
  uint32_t *new_dests = (uint32_t *)aligned_alloc(4096, new_N * sizeof(*(edges.dests)));
  uint32_t *new_vals = (uint32_t *)aligned_alloc(4096, new_N * sizeof(*(edges.vals)));
  uint32_t * vals = (uint32_t *)edges.vals;
  uint32_t * dests = (uint32_t *)edges.dests;
  ...
  int edgenum=0;
        ...
        printf("test: after modifying, the edge number is %d\n",edgenum);
  release_all_locks(task_id, true, GENERAL);
  gettimeofday(&end,NULL);
  graphstore::print_time_elapsed("double_list: ", &start, &end);
}  */ 
...
void PMA::double_list(uint64_t task_id, std::vector<uint_t> &sub_counts, uint_t num_elements) {
  ...
  edges.N = new_N;
  int num_pages=edges.N/1024;
  int edgenum=0;

  uint32_t * p0_dests=(uint32_t *)aligned_alloc(4096,new_N*sizeof(*edges.dests));
  uint32_t * p0_vals=(uint32_t *)aligned_alloc(4096,new_N*sizeof(*edges.vals));
 
  void* new_addr_dests=(void*)p0_dests;
  void* old_addr_dests=(void*)edges.dests;

  void* new_addr_vals=(void*)p0_vals;
  void* old_addr_vals=(void*)edges.vals;
  printf("db is set to %d\n",double_granularity);
   for (int i = 0; i < num_pages/(2*double_granularity);i++) {
    void* p1_dests=mremap(old_addr_dests,4096*double_granularity,4096*2*double_granularity,MREMAP_MAYMOVE | MREMAP_FIXED,new_addr_dests);
    void* p1_vals=mremap(old_addr_vals,4096*double_granularity,4096*2*double_granularity,MREMAP_MAYMOVE | MREMAP_FIXED,new_addr_vals);
    if (p1_dests == MAP_FAILED) {
        perror("mremap");
        exit(-1);
    }
    if (p1_vals == MAP_FAILED) {
        perror("mremap");
        exit(-1);
    }
    uint32_t* old_addr1=(uint32_t*)old_addr_dests;
    uint32_t* new_addr1=(uint32_t*)new_addr_dests;
    old_addr1+=1024*double_granularity;
    new_addr1+=2048*double_granularity;
    old_addr_dests=(void*)old_addr1;
    new_addr_dests=(void*)new_addr1;

    uint32_t* old_addr2=(uint32_t*)old_addr_vals;
    uint32_t* new_addr2=(uint32_t*)new_addr_vals;
    old_addr2+=1024*double_granularity;
    new_addr2+=2048*double_granularity;
    old_addr_vals=(void*)old_addr2;
    new_addr_vals=(void*)new_addr2;
  }
  //printf("here remap over\n");
  edges.dests=p0_dests;
  edges.vals=p0_vals; 
  for(int i=1024*double_granularity;i<edges.N;i+=2*1024*double_granularity){
    for(int k=i;k<i+1024*double_granularity;k++){
      edges.dests[k]=NULL_VAL;
    }
  }
  ...
  //memset((void*)(edges.items+(new_N / 2)), 0, sizeof(edge_t) * (new_N / 2) );
  //modified by ghr
  //printf("here modify offset start\n");  
  int vk=0,vm=0;
  uint32_t node_indexk;
  uint32_t node_indexm;
  for(int i=0;i<num_pages/(2*double_granularity);i++){
    vk=(2*i+1)*1024*double_granularity-1;
    for(;vk>(2*i)*1024*double_granularity;vk--){
      if(edges.dests[vk]==SENT_VAL||edges.vals[vk]==NULL_VAL) {
        node_indexk=edges.vals[vk];
        if (node_indexk == NULL_VAL) {
        node_indexk = 0;
        }
        break;
      }
    ...
    if(i==0){
      ...
      for(int j=node_indexm;j<=node_indexk;j++){
          nodes[j].beginning+=i*1024*double_granularity;
          nodes[j-1].end+=i*1024*double_granularity;
          if(j==nodes.size()-1){
  ...
  /* for(int i=1;i<10;i++){
      printf("%d %d!\n",nodes[i-1].beginning,nodes[i-1].end);
 ...
 //printf("here modify offset over\n");
  gettimeofday(&end,NULL);
  long int starttime=1000000*start.tv_sec+start.tv_usec-record;
  long int endtime=1000000*end.tv_sec+end.tv_usec-record;
  printf("start time is %.2f, end time is %.2f\n", (float)starttime/1000000,(float)endtime/1000000);
  graphstore::print_time_elapsed("double_list: ", &start, &end); 
  release_all_locks(task_id, true, GENERAL);
} 
...
#endif
  pair_int ends = which_locks_for_leaf(src);  //the vertices that are impacted by locking src
#if ENABLE_PMA_LOCK == 1
  //printf("grabbing locks %d through %d, by %lu with reason %d\n", ends.x, ends.y, get_worker_num(), reason);
  for (uint32_t i = ends.x; i <= ends.y; i++) {
    if (!nodes[i].lock.try_lock(task_id, reason)) {
...
void PMA::insert(uint64_t task_id, uint_t index, uint32_t elem_dest, uint32_t elem_val, uint32_t src
  ...
  // density >= (((double) edges.logN - 1) / edges.logN)) {
    ...
    if (len <= edges.N) {
      ...
      //printf("test: the edge array length is %d, len here is %d\n",edges.N,len);
      //printf("test: density is %f, density_b here is %f\n",density,density_b);
      ...
      //if (edges.N == orig_n) {
        ...
        printf("test: after double, the edge number is %d\n",edgenum);
        density_count = get_density_count(&edges, 0, edges.N);
        printf("test: after double, the density count is %d\n",density_count);
        printf("------------------------\n\n");
        release_all_locks(task_id, true, GENERAL);
        exit(-1); */
...
PMA::PMA(uint32_t init_n, int db) {
  next_task_id = 1;
  double_granularity=db;
  ...
  //modified by ghr, here needs to be aligned to 4KB
  edges.vals = (uint32_t *)aligned_alloc(4096, edges.N * sizeof(*(edges.vals)));
  edges.dests = (uint32_t *)aligned_alloc(4096, edges.N * sizeof(*(edges.dests)));
