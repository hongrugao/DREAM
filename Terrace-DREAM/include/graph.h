...
static inline void unlock(uint32_t *data)
{
  ...
  class Graph {
    public:
      ...
      Graph(uint32_t size,int db); // create a graph with the given size (#num nodes)
...
*/
  ...
  Graph::Graph(uint32_t size,int db) : second_level(size,db), num_vertices(size) {
  ...
  bool Graph::check_in_place_dup(const vertex s, const vertex d) const {
	...
	void Graph::add_inplace(vertex *srcs, vertex *dests, uint32_t idx,
													std::vector<uint32_t>& parts, uint8_t *array_pma,
													uint8_t *array_btree, uint8_t *array_btree_node) {
#endif              //add some edges of vertex i

		vertex s = srcs[parts[idx]];
		std::pair<uint32_t, uint32_t> range = {parts[idx], parts[idx+1]};
		uint32_t size = range.second - range.first;   //incoming new edges of vertex i
		...
		if (degree == 0) { // it's a new src. Copy neighbors to in place
			...
			memcpy(vertices[s].neighbors, &dests[range.first], cnt*sizeof(vertex));    //first add to in place
#if WEIGHTED
			...
			uint32_t in_place_limit = degree < NUM_IN_PLACE_NEIGHBORS ? degree :
				NUM_IN_PLACE_NEIGHBORS;
			vertex *new_in_place = (vertex*)calloc(degree + size, sizeof(vertex));  //a new array sized current egdes plused incoming edges for vertex i
#if WEIGHTED
			...
			memcpy(vertices[s].neighbors, new_in_place, cnt*sizeof(vertex)); //put the smaller edges in the vertex_neighbor array
#if WEIGHTED
...
// add edge in batch
	...
	void Graph::add_edge_batch(vertex *srcs, vertex *dests, uint32_t edge_count,
		...
		std::vector<uint32_t> parts;       //build an offset-like array?   this is wrong that 1 1 1 3 omit 2
    ...
    record=1000000*start1.tv_sec+start1.tv_usec;
    ...
    print_time_elapsed("Inserting edges to PMA: ", &start1, &end1); 
		// insert edges from sec list to b-tree 
    
    printf("start with btree\n");
		parallel_for (uint32_t i = 0; i < parts.size()-1; i++) {
