 ...
 */
ThreadPool::ThreadPool(const int NUM_OF_THREADS, bool lock_search, int double_granularity,float eta,uint32_t init_num_nodes, int partitions_per_domain)
    : finished(false) {
...
void ThreadPool::stop() {
  ...
  printf("The double time = %ld ms \n", doubleTime/1000);

  cout << "The overall updating  time: " << chrono::duration_cast<chrono::milliseconds>(end - s).count() << "ms"<< endl;
  cout <<endl;
  thread_pool.clear();
}
