...
void PCSR::double_list() {
  struct timeval startD,endD,start1,end1;
  ...
  const int block_size = 512*double_granularity;
  const int region_count=num_pages/(2*double_granularity);
  vector<uint32_t> high_degree_nodes;
  vector<bool> hot_regions(region_count, false);
  
  
  for(size_t i = 0; i < node_count; i++) {
    max_degree = max(max_degree, nodes[i].num_neighbors);
  }
  
  
  const double threshold = max_degree * double_eta;
  for(size_t i = 0; i < node_count; i++) {
    if(nodes[i].num_neighbors > threshold) {
      high_degree_nodes.push_back(i);
      int start_segment_id=nodes[i].beginning/block_size;
      int end_segment_id=nodes[i].end/block_size;
      for(int j=start_segment_id;j<=end_segment_id;j++){
        hot_regions[j]=true;
      }
    }
  }
  uint32_t uniform_new_space=0;
  uint32_t remaining_space=0;

  const int hot_region_count = std::count(hot_regions.begin(), hot_regions.end(), true);
  
  uniform_new_space = region_count / hot_region_count; 
  remaining_space =  region_count % hot_region_count;   
  vector<uint32_t> space_assignment(region_count, 0);
  for(int i=0;i<region_count;i++){
    if(hot_regions[i]){
      space_assignment[i]=uniform_new_space*block_size;
    }
  }
  space_assignment[space_assignment.size()-1]+=remaining_space*block_size;

  vector<uint32_t> remapping_positions(hot_regions.size(), 0);
  remapping_positions[0]=0;
  for(int i=1;i<region_count;i++){
    if(hot_regions[i-1]){
      remapping_positions[i]=remapping_positions[i-1]+block_size+space_assignment[i-1];
    }else{
      remapping_positions[i]=remapping_positions[i-1]+block_size;
    }
  }  
  ...
  /* int hot_region[8]={0,1,0,1,1,0,0,0};
  vector<uint32_t> space(8,0);
  int space_reserving=8/3;
  int remains=8%3;

  for(int i=0;i<8;i++){
    if(hot_region[i]){
      space[i]=space_reserving;
    }
  }
  space[space.size()-1]+=remains;
  int remapping_locations[8]={0,0,0,0,0,0,0,0};
  remapping_locations[0]=0;
  for(int i=1;i<8;i++){
    if(hot_region[i-1]){
      remapping_locations[i]=remapping_locations[i-1]+1+space_reserving;
    }else{
      remapping_locations[i]=remapping_locations[i-1]+1;
    }
  }
  for(int i=0;i<8;i++){
    cout<<remapping_locations[i]<<" ";
  }
  cout<<endl;
  exit(-1); */
  ...
  long int edgeredistribution=1000000*(endD.tv_sec-start1.tv_sec)+(endD.tv_usec-start1.tv_usec);
  long int reconstruction=1000000*(endD.tv_sec-startD.tv_sec)+(endD.tv_usec-startD.tv_usec);
  
  long int starttime=1000000*startD.tv_sec+startD.tv_usec-record;
  long int endtime=1000000*endD.tv_sec+endD.tv_usec-record;
  printf("start time is %.2f, end time is %.2f\n", (float)starttime/1000000,(float)endtime/1000000);
  //printf("%.2f\n",(float)additional/reconstruction);
  //printf("the proportion of edge redistribution/reconstruction is %.4f\n",(float)edgeredistribution/reconstruction);
  doubleTime+=reconstruction;
}
